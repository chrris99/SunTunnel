...
public:
	void setUniformMaterials(const std::vector<Material*>& materials) {
		...
		for (unsigned int mat = 0; mat < materials.size(); mat++) {
			sprintf_s(name, "materials[%d].ka", mat); setUniform(materials[mat]->ka, name);
			sprintf_s(name, "materials[%d].kd", mat); setUniform(materials[mat]->kd, name);
			sprintf_s(name, "materials[%d].ks", mat); setUniform(materials[mat]->ks, name);
			sprintf_s(name, "materials[%d].shininess", mat); setUniform(materials[mat]->shininess, name);
			sprintf_s(name, "materials[%d].F0", mat); setUniform(materials[mat]->F0, name);
			sprintf_s(name, "materials[%d].rough", mat); setUniform(materials[mat]->rough, name);
			sprintf_s(name, "materials[%d].reflective", mat); setUniform(materials[mat]->reflective, name);
		}
	}
	...
	void setUniformObjects(const std::vector<Sphere*>& objects) {
		...
		for (unsigned int o = 0; o < objects.size(); o++) {
			sprintf_s(name, "objects[%d].center", o);  setUniform(objects[o]->center, name);
			sprintf_s(name, "objects[%d].radius", o);  setUniform(objects[o]->radius, name);
		}
	}
};
